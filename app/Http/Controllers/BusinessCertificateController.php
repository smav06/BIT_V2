...
class BusinessCertificateController extends Controller
{
	public function index($typeofview){
		...
		$business = DB::table('t_business_information AS BUSINESS')
			->join('t_bf_barangay_clearance AS BC', 'BC.BUSINESS_ID', 'BUSINESS.BUSINESS_ID')
			->select(
				'BUSINESS.BUSINESS_ID'
				,'BUSINESS.BUSINESS_NAME'
			...
			->get();
		if($typeofview == "BusinessClearance")
			return view('certificateandforms.business', compact('business'));
	...
	public function Printfm_bc_001d(Request $request){
		...
		$insert = DB::table('t_issuance')
				->insert(array(
					'ISSUANCE_TYPE_ID' => 11
					,'BUSINESS_ID' => $business_main_id
					,'ISSUANCE_PURPOSE' => 'Barangay Clearance Tricycle'
					,'ISSUANCE_DATE' => date('Y-m-d')
					,'ISSUANCE_NUMBER' => $issuance_number
					,'RECEIVED_BY' => session('session_full_name')
					,'STATUS' => "Issued"
		...
		return response()->json(['issuance_number' => $issuance_number] );
	}
