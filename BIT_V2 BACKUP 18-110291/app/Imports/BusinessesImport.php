...
class BusinessesImport implements ToModel,WithStartRow
{
    ...
    */
    ...
    {
        //
        $get_check_nature = DB::table('r_business_nature')->where('BUSINESS_NATURE_NAME',$row[2])->value('BUSINESS_NATURE_ID');
        ...
        if($get_check_nature == "" &&  $get_check_zone == ""){
            ...
            $get_last_business_id = DB::table('t_business_information')->insertGetId([                        
                ...
                "BUSINESS_OR_ACQUIRED_DATE"    => preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/" ,$row[8], $matches) == true ? $row[8] : 
            ...
            DB::table('t_business_approval')->insert([
                ...
                "STATUS"   =>  $row[37],
                "APPROVED_BY"   =>  $row[35],
                "DATE_APPROVED"   =>  preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/" ,$row[36], $matches) == true ? $row[36] : 
                \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[36])->format('Y-m-d'),
            ]);
            echo "success";
        }else{
