...
<script>
	...
	$('#tbl_resident_lst_1').on('click', '#btnAddInfant', function(){
		...
		//set value
		$("input[id='txt_nb_id']").val(row.attr("id"));
		$('#txt_name').text(name);
		$('#txt_CRUD_status').val("Add_Infant");
		//modal & button
...
<div id="content" class="content">
	...
	<ul class="nav nav-pills">
		...
		<li class="nav-items">
			<a href="#nav-pills-tab-2" data-toggle="tab" class="nav-link" >
				...
				<span class="d-sm-block d-none">Add Infant Record</span>
			</a>
		</li>
		<li class="nav-items">
			<a href="" class="nav-link" >
				<span class="d-sm-block d-none">Infant List</span>
			</a>
		</li>
	</ul>
	<!-- end nav-pills -->
	...
	<div class="tab-content">
		...
		<div class="tab-pane fade active show" id="nav-pills-tab-1">
			...
			<div class="panel panel-inverse">
				...
				<div class="panel-heading">
					...
					<h4 class="panel-title">Tag Resident as Infant</h4>
				</div>
				<!-- end panel-heading -->
				...
				<div class="panel-body">
					...
					{{--<table id="tbl_resident_lst_1" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr class="gradeC" id="{{$row->NEWBORN_ID}}">
								...
								<td width="20%">
									<button type="button" class="btn btn-lime m-r-5 m-b-5" id="btnAddInfant">
										<i class="fas fa-lg fa-fw m-r-10 fa-tag"></i>Add Infant Record
									</button>
								</td>
							</tr>
							@endforeach
		...
		{{-- NAV PILLS TAB 2 - ADD Infant RECORD --}}
		<div class="tab-pane fade " id="nav-pills-tab-2">
			<div class="panel panel-inverse">
				...
				<div class="panel-heading">
					...
					<h4 class="panel-title">Infant Resident</h4>
				</div>
				<!-- end panel-heading -->
				<div class="alert alert-yellow fade show">
					<button type="button" class="close" data-dismiss="alert">
				...
				<div class="panel-body">
					<table id="tbl_resident_lst_2" class="table table-striped table-bordered">
						...
						<tbody>
							@foreach($InfantLst as $row)
							<tr class="gradeC" id="{{$row->INFANT_ID}}">
								<td>{{$row->FIRSTNAME}} {{$row->MIDDLENAME}} {{$row->LASTNAME}} </td>
								<td>{{$row->ADDRESS_UNIT_NO}} {{$row->ADDRESS_PHASE}} {{$row->ADDRESS_BLOCK_NO}} {{$row->ADDRESS_HOUSE_NO}} {{$row->ADDRESS_STREET}} {{$row->ADDRESS_SUBDIVISION}} {{$row->ADDRESS_BUILDING}}</td>
								<td>{{$row->DATE_OF_BIRTH}}</td>
								<td width="20%">
									<button type="button" class="btn btn-success m-r-5 m-b-5" id="btnUpdateInfant" data-toggle='modal' data-target='#modal-Infant'>
										<i class="fas fa-lg fa-fw m-r-10 fa-tag"></i>Update Infant
									</button>
								</td>
							</tr>
							@endforeach	
